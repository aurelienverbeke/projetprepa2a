...
def match(taillePlateau, evaluations):
...
def test_evaluation(taillePlateau, nombreParties, *args):
    ...
    for indexPartie in range(nombreParties):
        iaGagnante = match(taillePlateau, args)
        nombreVictoiresParJoueur[iaGagnante.index] += 1
        print(f"Partie {indexPartie+1} terminee, victoire : {iaGagnante.index}")
